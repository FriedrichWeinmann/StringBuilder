...
$parentModule = 'StringBuilder'
if (-not $ModuleName) { $ModuleName = 'StringBuilder.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\StringBuilder'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\StringBuilder.psd1"
$prereqHash = @{
