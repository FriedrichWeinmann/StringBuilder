...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'StringBuilder.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'StringBuilder.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'StringBuilder.Client.ProtectedToken'
